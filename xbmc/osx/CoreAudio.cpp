...
const char* StreamDescriptionToString(AudioStreamBasicDescription desc, std::string& str)
{
  ...
  switch (desc.mFormatID)
  {
    case kAudioFormatLinearPCM:
      str = StringUtils::Format("[%4.4s] %s%sInterleaved %u Channel %u-bit %s %s(%uHz)",
                 fourCC,
    ...
    case kAudioFormatAC3:
      str = StringUtils::Format("[%4.4s] AC-3/DTS (%uHz)",
                 fourCC,
                 (desc.mFormatFlags & kAudioFormatFlagIsBigEndian) ? "BE" : "LE",
                 (UInt32)desc.mSampleRate);
      break;
    case kAudioFormat60958AC3:
      str = StringUtils::Format("[%4.4s] AC-3/DTS for S/PDIF (%uHz)", fourCC, (UInt32)desc.mSampleRate);
      break;
    default:
      str = StringUtils::Format("[%4.4s]", fourCC);
      break;
  }
  return str.c_str();
}
...
AudioDeviceID CCoreAudioHardware::FindAudioDevice(const std::string& searchName)
{
  ...
  if (searchName == "Default Output Device")
  {
...
const char* CCoreAudioDevice::GetName(std::string &name)
{
  ...
  name.resize(size);
  OSStatus ret = AudioDeviceGetProperty(m_DeviceId, 0, false, kAudioDevicePropertyDeviceName, &size, (void*)name.data());
  if (ret)
  {
...
OSStatus CCoreAudioUnit::OnRender(AudioUnitRenderActionFlags *ioActionFlags, const AudioTimeStamp *inTimeStamp, UInt32 inBusNumber, UInt32 inNumberFrames, 
...
OSStatus CCoreAudioUnit::RenderCallback(void *inRefCon, AudioUnitRenderActionFlags *ioActionFlags, const AudioTimeStamp *inTimeStamp, UInt32 inBusNumber, 
...
OSStatus CAUOutputDevice::OnRender(AudioUnitRenderActionFlags *ioActionFlags, const AudioTimeStamp *inTimeStamp, UInt32 inBusNumber, UInt32 inNumberFrames, 
AudioBufferList *ioData)
{
